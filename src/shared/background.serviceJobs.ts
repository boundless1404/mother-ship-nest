    ...
    try {
      ...
      const emailJob = (await dbManager
        ...
        .select(
          `
            case when 
                    exists(select * from email where priority = '${EmailPriority.IMMEDIATE}' and "email"."retryCount" <=2 and "email"."deletedAt" is null)
                then (select jsonb_agg(jsonb_build_object(
                    'id', "email"."id",
                    'body', "email"."body",
                    'attachmentFileUrls', "email"."attachmentFileUrls",
                ...
                when exists(select * from email where priority = '${EmailPriority.REGULAR}' and "email"."retryCount" <=2 and "email"."deletedAt" is null)
                then (select jsonb_agg(jsonb_build_object(
                    'id', "email"."id",
                    'body', "email"."body",
                    'attachmentFileUrls', "email"."attachmentFileUrls",
                ...
                when 
                    exists(
                            select * from email where priority = '${EmailPriority.DELAYED}' and "email"."retryCount" <=2 and "sendAt" is not null and now() >= "email"."sendAt"
                            and "email"."deletedAt" is null
                          )
                 then (select jsonb_agg(jsonb_build_object(
                        'id', "email"."id",
      ...
      if (emailJob && emailJob.emails?.length) {
        ...
        const emails: MailDataRequired[] = await Promise.all(
          emailJob.emails.map(async (email) => {
            // TODO: implement file attachment sending
