...
export default class Wallet_Transaction {
   @PrimaryGeneratedColumn({type: 'bigint'})
    ...
    // relations
    @ManyToOne(() => Wallet, (wallet) => wallet.destination_wallet_transactions, { onDelete: 'CASCADE'})
    @JoinColumn({ name: 'destination_wallet_id'})
    destination_wallet: Wallet;

    @ManyToOne(() => Wallet, (wallet) => wallet.source_wallet_transactions, { onDelete: 'CASCADE'})
